...
export async function queryWindow({ key, fromFull, toFull, terminal }) {
  ...
  return toRows(raw).filter((r) => {
    const t = r.estimated || r.scheduled;
    return t && t >= fromFull && t <= toFull;
  });
}
...
// origin(부분일치)·terminal(T1/T2) 로 추가로 걸러낼 수 있습니다.
export async function listUpcoming({ key, withinMinutes = 120, origin, terminal, now = new Date() }) {
  ...
  rows.sort((a, b) => (a.estimated || a.scheduled || "").localeCompare(b.estimated || b.scheduled || ""));
  return rows;
}
